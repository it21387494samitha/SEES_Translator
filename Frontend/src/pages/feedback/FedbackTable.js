...
const FeedbackTable = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const res = await axios.post(`${url}/user/details`, null, {
        headers: {
  ...
  const getData = async () => {
    ...
    const url = `http://localhost:4000/feedback/user?userId=${userId}`;
    axios
      .get(url, {
        headers: {
          authorization: `Bearer ${token}`,
  ...
  const confirmDelete = () => {
    const token = localStorage.getItem("accessToken");
    axios
      .delete(`${url}/feedback/delete/${selectedFeedback}`, {
        headers: {
          authorization: `Bearer ${token}`,
        },
      })
  ...
  const updateFeedback = () => {
    ...
    axios
      .put(
        `${url}/feedback/update/${selectedFeedback._id}`,
        {
