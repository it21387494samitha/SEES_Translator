...
export default function BadwordFeature(userId) {
  ...
  async function getDetails() {
    ...
    try {
      const response = await axios.get(`${url}/bad/badpost`, { params: id });
  ...
  useEffect(() => {
    getDetails(); // Initial fetch

    const intervalId = setInterval(() => {
      getDetails(); // Fetch data every 1 second
    }, 1000);
